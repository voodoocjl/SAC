...
class Agent:
    def __init__(self, x, y):
        self.pos_old = np.array(
            [random.uniform(-ini_loc, ini_loc), random.uniform(-ini_loc, ini_loc)]
        )  # 智能体的位置, [x, y]
        self.vel_old = np.array(
            [random.uniform(-maxSpeed, maxSpeed), random.uniform(-maxSpeed, maxSpeed)]
        )  # 智能体的速度, [Vx, Vy]
        self.target = np.array(
            [
                random.uniform(-ini_loc, ini_loc) + x,
                random.uniform(-ini_loc, ini_loc) + y,
            ]
        )  # 智能体的目标位置
        self.pos_new = self.pos_old  # new position, [x, y]
        self.vel_new = self.vel_old  # new velocity, [Vx, Vy]
